...
bgImage.src = '/image-assets/home bgm.png';
...
function drawModal(ctx) {
    ...
    if (activeModal === 'about') {
        ...
        ctx.font = 'italic 18px monospace'; ctx.fillStyle = '#ffd700';
        ctx.fillText("(BGM + Levels design)", x + w/2, startY + gap * 2 + 25);
...
function createStartOverlay() {
    ...
    btn.onclick = () => {
        ...
        // A. Trigger Fullscreen (only if NOT in iframe)
...
window.update = function() {
    ...
    if (window.canvasMenuVisible) { 
        ...
        for (const btn of canvasButtons) {
            ...
            // Simply use mouseIsOver since there is no player collision logic anymore
            btn.hot = btn.mouseIsOver;
        }
    }
};
